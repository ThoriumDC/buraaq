...
static LRESULT CALLBACK bq_gfx_wnd(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    if (m == WM_PAINT) {
        ...
        if (g_mdc && g_w > 0 && g_h > 0) {
            BitBlt(hdc, 0, 0, g_w, g_h, g_mdc, 0, 0, SRCCOPY);
        }
        EndPaint(h, &ps);
        return 0;
    }
...
static void gfx_free_window(void) {
    if (g_mdc) {
        if (g_old) SelectObject(g_mdc, g_old);
        g_old = NULL;
...
static int gfx_open_window(const char *title) {
    ...
    g_bmp = CreateDIBSection(g_mdc, &bmi, DIB_RGB_COLORS, &g_bits, NULL, 0);
    if (!g_bmp || !g_bits) {
        ReleaseDC(g_hwnd, hdc);
        gfx_free_window();
        return 0;
    }
    g_old = (HBITMAP)SelectObject(g_mdc, g_bmp);
    ReleaseDC(g_hwnd, hdc);
    if (g_own_px && g_px) free(g_px);
    g_px = (uint32_t *)g_bits;
    g_own_px = 0;
...
void buraaq_gfx_flip(void) {
    ...
    if (g_hwnd) {
        ...
        if (g_hwnd) {
            HDC hdc = GetDC(g_hwnd);
            if (hdc && g_mdc) BitBlt(hdc, 0, 0, g_w, g_h, g_mdc, 0, 0, SRCCOPY);
